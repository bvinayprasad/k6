...
func newBaseClient(signer HTTPRequestSigner, dispatcher HTTPRequestDispatcher) BaseClient {
	...
	//fmt.Println("inside new bse client")
	//fmt.Println(getNextSeed())
	return BaseClient{
...
//fmt.Println("got into default http dispatched client")
...
//fmt.Println("got into default base client")
	dispatcher := defaultHTTPDispatcher()
	signer := DefaultRequestSigner(provider)
	//fmt.Println("done with default base client")
...
func (client BaseClient) CallWithDetails(ctx context.Context, request *http.Request, details ClientCallDetails) (response *http.Response, err error) {
	...
	//fmt.Println("after prepare request")
	//Intercept
	...
	//fmt.Println("request before sending")
	//fmt.Println(request)
	//fmt.Println(request.Header.Get(requestHeaderXContentSHA256))
